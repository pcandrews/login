...
// Create (CRUD)
func (e Empleado) Crear() Empleado {
	//var datetime = time.Now()
	//datetime.Format(time.RFC3339)
	db.Exec("ALTER TABLE `personas` AUTO_INCREMENT = 1;")
	//e.Persona.CreadoEn = datetime
...
// Read (CRUD)
func (e Empleado) Leer(id uint) Empleado {
	var p Persona
	db.First(&e, id)
...
func (e Empleado) LeerTodos() []Empleado {
	...
	//deberia esta haecho con gorm, esta hecho con sql, luego agregar el metodo de gorm
	//db.Find(&todosE)
	//db.Joins("LEFT JOIN empleados ON empleados.id_persona = personas.id_persona").Find(&todosE)
...
// Update (CRUD)
func (e Empleado) Actualizar(id uint) Empleado {
	var datetime = time.Now()

	datetime.Format(time.RFC3339)

	db.Exec("UPDATE empleados SET actualizado_en=? WHERE empleados.id_empleado=?", datetime, id).Find(&e)

	return e
}

// Delete (Softdelete) (CRUD)
func (e Empleado) Eliminar(id uint) Empleado {
	var datetime = time.Now()
	...
	//db.Debug().Exec("UPDATE empleados SET eliminado_en=? WHERE empleados.id_empleado=?", datetime, id).Find(&aux)

	// Disable Logger, don't show any log even errors
	//db.LogMode(false)

	db.Exec("UPDATE empleados SET eliminado_en=? WHERE empleados.id_empleado=?", datetime, id).Find(&e)

	return e
}

/*
func (e Empleado) EnviarNombre() string {
	return e.NombresPersona
}
*/
...
func ObtenerTodosLosEmpleados(c *gin.Context) {
	var resultado gin.H
	//var err error
	//este codigo no es elegante, deberia tener mas sentido. Pero funciona.
	//arrelgar  en el futuro
	var e Empleado
	var todosE []Empleado
